...
export default function ModeSwitcher({ mode, onChange }) {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 rounded-lg border border-ink-200 px-2.5 py-1 text-sm text-ink-600 hover:bg-ink-50 dark:border-ink-700 dark:text-ink-300 dark:hover:bg-ink-700"
      >
        ...
        <div className="absolute right-0 top-full z-30 mt-1 w-64 card p-1.5 shadow-panel">
          {MODES.map(({ id, label, desc, Icon }) => (
            <button
              ...
              className={`flex w-full items-start gap-2.5 rounded-md px-2.5 py-2 text-left hover:bg-ink-50 dark:hover:bg-ink-700 ${
                mode === id ? "bg-brand-50 dark:bg-brand-500/15" : ""
              }`}
            >
