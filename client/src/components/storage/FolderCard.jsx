...
export default function FolderCard({ folder, canManage, onOpen, onEdit, onDelete, onAssetDrop, onFileDrop }) {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`card flex cursor-pointer items-center gap-3 p-4 transition-colors hover:border-brand-300 dark:hover:border-brand-500 ${
        dragOver ? "border-brand-400 bg-brand-50 dark:bg-brand-500/15" : ""
      }`}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <p className="truncate text-sm font-medium text-ink-800 dark:text-ink-100">{folder.name}</p>
          {folder.visibility === "RESTRICTED" && <Lock className="h-3 w-3 shrink-0 text-accent-500" />}
        </div>
        {dragOver ? (
          <p className="text-xs font-medium text-brand-600 dark:text-brand-400">Drop to move here</p>
        ) : (
