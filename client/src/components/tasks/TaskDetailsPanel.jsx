...
export default function TaskDetailsPanel({ open, task, workspaceId, statuses, onClose, onEdit }) {
  ...
  return (
    <div className="absolute inset-0 z-40">
      <div
        className={`absolute inset-0 bg-ink-900/50 transition-opacity duration-300 dark:bg-black/60 ${show ? "opacity-100" : "opacity-0"}`}
        onClick={onClose}
      />
      <div
          ...
          <div className={`space-y-5 ${expanded ? "mx-auto w-full max-w-6xl" : ""}`}>
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-1.5">
                {task.isPinned && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-brand-50 px-2 py-0.5 text-xs font-medium text-brand-700 dark:bg-brand-500/15 dark:text-brand-300">
                    <Pin className="h-3 w-3" /> Pinned
                  </span>
                )}
                {task.isSnoozed && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-ink-100 px-2 py-0.5 text-xs font-medium text-ink-500 dark:bg-ink-700">
                    <Moon className="h-3 w-3" /> Snoozed
                  </span>
                )}
                <TierBadge tier={task.tier} />
              </div>
              <h3 className="text-lg font-semibold text-ink-900 dark:text-ink-50">{task.title}</h3>
            </div>
            ...
            <div className={`grid gap-4 ${expanded ? "grid-cols-4" : "grid-cols-2"}`}>
              ...
              <div>
                ...
                {task.dueDate ? (
                  <span className={`inline-flex items-center gap-1.5 text-sm font-medium ${overdue ? "text-red-500" : "text-ink-700 dark:text-ink-200"}`}>
              ...
              <div>
                ...
                {task.assignee ? (
                  <span className="inline-flex items-center gap-2 text-sm font-medium text-ink-700 dark:text-ink-200">
            ...
            <div className="border-t border-ink-100 pt-4 dark:border-ink-700">
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={uploading}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-ink-200 px-2.5 py-1 text-xs font-medium text-ink-600 hover:bg-ink-50 disabled:opacity-50 dark:border-ink-700 dark:text-ink-300 dark:hover:bg-ink-700"
                >
              ...
              {attachmentsLoading ? (
                ...
                <ul className="space-y-1.5">
                  {attachments.map((asset) => (
                    <li
                      key={asset.id}
                      className="group flex items-center gap-2.5 rounded-lg border border-ink-100 px-2.5 py-2 hover:bg-ink-50 dark:border-ink-700 dark:hover:bg-ink-700"
                    >
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-ink-100 text-ink-500 dark:bg-ink-700">
                      ...
                      <button
                        ...
                        title="Remove"
                        className="shrink-0 rounded-md p-1 text-ink-300 opacity-0 transition-opacity hover:bg-red-50 hover:text-red-500 group-hover:opacity-100 focus:opacity-100 dark:hover:bg-red-500/10 dark:hover:text-red-400"
                      >
            ...
            {(task.createdAt || task.updatedAt) && (
              <div className="border-t border-ink-100 pt-4 text-xs text-ink-400 dark:border-ink-700">
