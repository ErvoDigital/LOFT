...
Access is plain `requireWorkspaceMember()` — no Folder-style restricted-visibility allow-list, matching the majority pattern (Calendar/Tasks/Chat channels) rather than Storage's restricted-folder model. Delete is creator-or-ADMIN only; rename is open to any member.
